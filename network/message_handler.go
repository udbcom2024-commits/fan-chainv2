...
func (s *Server) handleBlocks(peer *Peer, msg *Message) {
	...
	// 【P2协议-优先处理】backfill区块必须在分类之前处理
	s.syncMu.Lock()
	isBackfillEarly := s.backfillInProgress
	backfillTargetEarly := s.backfillTargetHeight
	cpHeightEarly := s.checkpointHeight
	s.syncMu.Unlock()

	if isBackfillEarly && len(blocks.Blocks) > 0 && s.saveBlockOnly != nil {
		firstH := blocks.Blocks[0].Header.Height
		lastH := blocks.Blocks[len(blocks.Blocks)-1].Header.Height
		if lastH < cpHeightEarly {
			log.Printf("📦 【P2-Backfill】Processing %d blocks (%d-%d)", len(blocks.Blocks), firstH, lastH)
			savedCount := 0
			var lowestSaved uint64 = ^uint64(0)
			for _, block := range blocks.Blocks {
				if block.Header.Height < cpHeightEarly {
					if err := s.saveBlockOnly(block); err != nil {
						log.Printf("⚠️ 【P2-Backfill】Failed #%d: %v", block.Header.Height, err)
					} else {
						savedCount++
			...
			if lowestSaved > backfillTargetEarly {
				log.Printf("📦 【P2-Backfill】Continuing from %d to %d", lowestSaved-1, backfillTargetEarly)
				s.requestBackfillBatch(peer, lowestSaved-1, backfillTargetEarly)
			} else {
				s.syncMu.Lock()
				s.backfillInProgress = false
				s.syncMu.Unlock()
				log.Printf("✅ 【P2协议】Backfill completed! Earliest: %d", lowestSaved)
			}
			return
		}
	}

	// 【修复】获取当前本地高度，验证收到的区块是否匹配预期
	...
	// 【checkpoint同步修复】如果有checkpointSyncFrom，允许从该高度开始接受区块
	s.syncMu.Lock()
	checkpointFrom := s.checkpointSyncFrom
	cpHeight := s.checkpointHeight
	// 【修复】如果当前高度已经超过checkpoint高度，说明向上同步完成，清除checkpointSyncFrom
	// 但是保留checkpointHeight用于backfill判断，直到backfill完成
	if checkpointFrom > 0 && currentLocalHeight >= cpHeight {
		log.Printf("📦 Forward sync completed: height %d >= checkpoint %d, clearing checkpointSyncFrom (keeping cpHeight for backfill)", currentLocalHeight, cpHeight)
		s.checkpointSyncFrom = 0
		checkpointFrom = 0
		// 不清除s.checkpointHeight，backfill需要用它
	}
	s.syncMu.Unlock()
	...
	for _, block := range blocks.Blocks {
		...
		if block.Header.Height == expectedNextHeight {
			...
			// 【用户方案】先存到数据库，等缺失的区块收齐后再连续应用
			log.Printf("📦 Block #%d 跳过期望#%d，先存库等收齐", block.Header.Height, expectedNextHeight)
			if s.saveBlockOnly != nil {
				s.saveBlockOnly(block)
			}
			// 继续处理后续区块（不break）
		}
	}

	// 【用户方案】检查数据库是否已有连续区块可应用
	if len(filteredBlocks) == 0 && s.getBlockRange != nil && s.addBlockSkipTimestamp != nil {
		log.Printf("📦 尝试从数据库应用连续区块...")
		for h := currentLocalHeight + 1; ; h++ {
			dbBlocks, err := s.getBlockRange(h, h)
			if err != nil || len(dbBlocks) == 0 {
				// 缺区块了，停止应用，等下次收齐
				if h == currentLocalHeight + 1 {
					log.Printf("📦 区块#%d不在数据库，继续请求...", h)
				}
				break
			}
			if err := s.addBlockSkipTimestamp(dbBlocks[0]); err != nil {
				log.Printf("⚠️ 应用区块#%d失败: %v", h, err)
				break
			}
			filteredBlocks = append(filteredBlocks, dbBlocks[0])
			log.Printf("✓ 从数据库应用区块#%d", h)
		}
	}
	...
	if s.addBlock != nil {
		...
		for i, block := range filteredBlocks {
			...
			// 【checkpoint回填】如果区块高度小于checkpoint高度，只保存到数据库，不更新链状态
			...
			if err != nil {
				...
				// 【先存后验】同步期间，即使addBlock失败也保存区块到数据库
				// 新节点小弟先信任大哥，参与共识前再校验
				if cpHeight > 0 && s.saveBlockOnly != nil {
					if saveErr := s.saveBlockOnly(block); saveErr == nil {
						log.Printf("📦 Block #%d saved (trust-first sync, will verify later)", block.Header.Height)
						lastAddedHeight = block.Header.Height
					}
				}
				// 添加失败不中断同步，继续尝试后续区块
...
func (s *Server) handleNewBlock(peer *Peer, msg *Message) {
	...
	// 【Archive/History节点修复】如果当前高度为0但已经应用了checkpoint，使用checkpoint高度
	s.syncMu.Lock()
	if currentHeight == 0 && s.checkpointHeight > 0 {
		currentHeight = s.checkpointHeight
		log.Printf("📌 Using checkpoint height %d as current height (latestBlock not yet set)", currentHeight)
	}
	s.syncMu.Unlock()
	...
	if incomingHeight == currentHeight+1 {
		...
		if s.verifyProposer != nil {
			...
			if latestBlock != nil {
				...
				if newBlock.Block.Header.Proposer != expectedProposer {
					...
					// 【简单多数认输】记录被拒绝的proposer，检查是否应该认输
					shouldSurrender, winningProposer := s.RecordRejectedProposer(incomingHeight, newBlock.Block.Header.Proposer)
					if shouldSurrender {
						log.Printf("🏳️ 【简单多数认输】Surrendering at height %d, accepting %s's block",
							incomingHeight, winningProposer[:10])
						// 直接接受这个区块
						if s.addBlock != nil {
							if err := s.addBlock(newBlock.Block); err != nil {
								log.Printf("Failed to add surrendered block #%d: %v", incomingHeight, err)
							} else {
								log.Printf("✓ Accepted surrendered block #%d from %s", incomingHeight, winningProposer[:10])
								// 清理旧记录
								s.CleanupRejectedProposers(incomingHeight)
							}
						}
					}
					return
		...
		if s.addBlock != nil {
			if err := s.addBlock(newBlock.Block); err != nil {
				...
				s.syncMu.Unlock()
				if cpHeight > 0 && s.saveBlockOnly != nil {
					if saveErr := s.saveBlockOnly(newBlock.Block); saveErr == nil {
						log.Printf("📦 Block #%d saved (trust-first sync)", incomingHeight)
					}
				}
			} else {
	...
	if incomingHeight == currentHeight && currentHeight > 0 {
		...
		if s.verifyProposer != nil && currentHeight > 1 {
			...
			// 情况A: 接收到的区块来自正确proposer，本地区块不是 -> 执行reorg
			...
			// 情况B: 接收到的区块来自错误proposer，本地区块是正确的 -> 拒绝
			if incomingProposer != expectedProposer && localProposer == expectedProposer {
				...
				// 【简单多数认输】记录被拒绝的proposer，检查是否应该认输（针对相同高度的区块）
				shouldSurrender, winningProposer := s.RecordRejectedProposer(currentHeight, incomingProposer)
				if shouldSurrender {
					log.Printf("🏳️ 【简单多数认输】Height %d: my VRF says %s but majority says %s, need REORG!",
						currentHeight, expectedProposer[:10], winningProposer[:10])
					// 执行reorg接受多数派的区块
					if s.performReorg != nil {
						if err := s.performReorg(currentHeight-1, newBlock.Block); err != nil {
							log.Printf("❌ SURRENDER REORG FAILED: %v", err)
						} else {
							log.Printf("✅ SURRENDER REORG SUCCESS: Accepted majority block at height %d", currentHeight)
							s.CleanupRejectedProposers(currentHeight)
						}
					}
				}
				return
	...
	if incomingHeight > currentHeight+1 {
		...
		checkpointFrom := s.checkpointSyncFrom
		cpHeight := s.checkpointHeight
		// 【修复】如果当前高度已经超过checkpoint高度，说明向上同步完成
		// 保留checkpointHeight用于backfill判断
		if checkpointFrom > 0 && currentHeight >= cpHeight {
			log.Printf("📦 Forward sync completed in handleNewBlock: height %d >= checkpoint %d", currentHeight, cpHeight)
			s.checkpointSyncFrom = 0
			checkpointFrom = 0
		}
		s.syncMu.Unlock()
...
func (s *Server) handleGetEarliestHeight(peer *Peer, msg *Message) {
	...
	var earliestHeight uint64 = 1 // 默认值
